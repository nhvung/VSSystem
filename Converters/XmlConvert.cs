...
namespace VSSystem
{
    public class XmlConvert
    {
        ...
        public static TResult DeserializeObjectFromBytes<TResult>(Stream stream, Encoding encoding = default)
        {
            try
            {
                if (encoding == null)
                    encoding = Encoding.UTF8;
                TResult result = default;
                StreamReader xmlStream = new StreamReader(stream, encoding);
                XmlSerializer serializer = new XmlSerializer(typeof(TResult));
                result = (TResult)serializer.Deserialize(xmlStream);
                return result;
            }
